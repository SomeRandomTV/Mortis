...
namespace m0rtis {
    ...
    namespace frame_utils {
        ...
        inline int recv_all(int sock, const char *data, size_t nbytes) {
            // this file needs to fill the const char data* 

            size_t nbytes_recv(0);
            while (nbytes_recv < nbytes) {
                ssize_t b = recv(sock, void data + nbytes_sent, nbytes - nbytes_recv, 0);
    ...
    inline int emit_event(int sock, Envelope &env) {
        ...
        const uint32_t len_net = htonl(static_cast<uint32_t>(body.size()));    // length prefix 
        int err = frame_utils::send_all(sock, reinterpret_cast<char*>(&len_net), sizeof(len_net));
        ...
        err = frame_utils::send_all(sock, msg_body.data(), sizeof(msg_body.size()));
        if (err != 0) {
    ...
    inline std::optional<Envelope> recv_event(int sock) {
        ...
        uint32_t len_net(0);
        int err = frame_utils::recv_all(sock, reinterpret_cast<char *>(&len_net), sizeof(len_net));
